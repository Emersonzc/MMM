...
def main():
    ...
    main_worker_stage2(args,log_s1_name,log_s2_name,subdir) 
...
def main_worker_stage1(args,log_s1_name,subdir):
    ...
    trainer = ClusterContrastTrainer_intra_modal(model)
    # ########################
    normalizer = T.Normalize(mean=[0.485, 0.456, 0.406],
                         std=[0.229, 0.224, 0.225])
    height=args.height
...
def main_worker_stage2(args,log_s1_name,log_s2_name,subdir,update_subdir=None):
    ...
    lr_scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=args.step_size, gamma=0.1)
    # Trainer
    trainer = ClusterContrastTrainer_inter_modal(model)
    for epoch in range(args.epochs):
        with torch.no_grad():
            if epoch == 0:
                # DBSCAN cluster
...
############################
    ...
    try:
        ...
        print("Don't save feature files!")

    # multi-memory matching
    from sklearn.metrics import adjusted_rand_score
    from sklearn.cluster import KMeans
    print("multi-memory matching------>")
    GT_rgb = np.load(args.data_dir+'/train_rgb_label.npy')
    GT_ir = np.load(args.data_dir+'/train_ir_label.npy')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data-dir', type=str, metavar='PATH',
                        default='/data/yxb/datasets/ReIDData/SYSU-MM01/')
